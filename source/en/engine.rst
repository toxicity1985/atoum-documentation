...
Several execution engines to run the tests (at class or methods level) are available. These are configurable via the ``@engine`` annotation. By default, the different tests run in parallel in PHP sub-processes, this is the ``concurrent`` mode.

Actually, there is three execution modes :
